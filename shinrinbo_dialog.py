...
from qgis.PyQt.QtWidgets import QApplication, QDialog, QFileDialog, QMessageBox, QSizePolicy
...
class _XlsxReadCancelled(Exception):
    pass
...
class ShinrinboDialog(QDialog, FORM_CLASS):
    ...
    def __init__(self, iface, parent=None):
        ...
        # ボタン行に伸縮スペースを入れて、既存レイアウトの寄せ方を保つ。
        # .ui側で<spacer>を宣言するとPyQt6のuicが
        # 「QSpacerItem(...): argument 1 has unexpected type 'Policy'」で
        # setupUi()自体を落とすため、ここでPython側から追加する。
        self.hboxModeToggle.insertStretch(0)
        size_policy = getattr(QSizePolicy, 'Policy', QSizePolicy)
        for button in (self.btnExecute, self.btnCancel, self.btnClose):
            button.setSizePolicy(size_policy.Expanding, size_policy.Fixed)
            self.hboxButtons.setStretch(self.hboxButtons.indexOf(button), 1)
        self.hboxButtons.insertSpacing(self.hboxButtons.indexOf(self.btnClose), 12)
    ...
    def _execute_initial(self):
        ...
        except _XlsxReadCancelled:
            self.progressBar.setRange(0, 100)
            self.btnExecute.setEnabled(True)
            self.btnCancel.setEnabled(False)
            self.labelStatus.setText('キャンセル')
            self.textResult.setPlainText('キャンセルしました。')
            return
        except Exception as e:
            self.progressBar.setRange(0, 100)
            self.progressBar.setValue(0)
